...
| `Interrupt` 超时 | 单题 50ms | `while(true)` 实测 60ms 内被杀；`ClearInterrupt` 后 VM 可复用 |
...
| `release.yml` | tag `v*` | 构建静态 Go 二进制 → 多阶段 Docker 构建 → push ghcr（`latest`/版本号/`sha`）→ `docker save \| gzip` → 创建 GitHub Release 并上传 `.tar.gz` |
