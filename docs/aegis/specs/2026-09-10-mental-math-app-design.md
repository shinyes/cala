...
> 上表是 **`scoring` 层**的分类语义（历史数据的判分依据，保持不变）。
> **service 层在其上再收窄**：文本答案与分数形式均被拒绝，
> 详见本节下方两条执行期修正。
...
> **执行期修正（2026-09-11，同日第二次收窄）：答案只接受整数或小数。**
>
> 分数形式（如 `a: "1/2"`）在**保存期与服务期均被拒绝**，
> 提示作者改写为等值小数（`1/2` → `0.5`）。
>
> 理由与文本答案同源，且更强：练习键盘没有除号（§5.5.4），
> 于是分数形式里**无限循环小数**（如 `1/3`）永远敲不出来，
> 作者必须自行配容差才行 —— 那是个容易忽略的坑。
> 只允许整数与小数后，**「作者写的答案一定能用练习键盘原样敲出」无条件成立**，
> 消除了最后一类「看起来合法但用户答不出」的题目。
>
> 判分基础不受影响：`1/2` 与 `0.5` 数值相等（交叉相乘），
> 因此**已落库的分数答案仍能被用户用小数答对**；
> 作者把分数改写成小数不改变题目对错。
>
> 兼容边界同文本答案：拒绝只在 service 层，
> **`scoring.Classify` 与 `ParseRational` 必须继续接受分数** ——
> 历史信封与错题重练靠它们判分。由 `TestScoringStillClassifiesTextAndFraction` 锁定。
>
> 实现判据：看**清洗后的原始字面量是否含 `/`**，而不是看信封 ——
> `0.5` 与 `1/2` 会生成数值相等但形状不同的信封（`5/10` 与 `1/2`），
> 按信封无法区分「作者写的是分数」与「作者写的是小数」。
...
POST   /api/auth/password            → { token }   # 改密；见下方说明
GET    /api/me
...
**`/auth/password` 的服务端行为**（2026-09-11 新增）：

请求体 `{ currentPassword, newPassword }`，需已认证。

1. **必须提供当前口令**。若只凭会话令牌放行，任何拿到令牌的人
   （设备失窃、令牌泄露）都能改密并**永久占有**账号 ——
   令牌本会过期，改密却把访问权延展到无限期。
2. 校验顺序：新口令长度 -> **校验当前口令** -> 新旧是否相同。
   当前口令必须先于「新旧相同」判定：否则用户把当前口令输错、
   且新口令恰等于那个错误输入时，会收到「新旧相同」，
   而真正的问题是当前口令不对，提示会把人引向错误的修正方向。
3. **新旧相同则拒绝**。否则接口返回成功却什么都没变，
   用户以为换掉了泄露的口令，实际没有 —— 比直接报错危险。
4. **成功后吊销该用户的全部会话**，并为当前设备签发新令牌并在响应中返回。
   吊销「全部」而非「其他设备」：排除当前会话需要额外条件，漏写就是把攻击者的会话留下。
   随即签发新令牌使当前设备无感续用，用户不会在改密后被踢下线。
5. 口令更新、会话吊销、新会话签发在**同一事务**内完成。
   分开执行会留下自相矛盾的状态（口令已改但旧会话仍有效，
   或口令已改但客户端收到错误而以为失败）。

**状态码约定（易错，务必遵守）**：当前口令不正确返回 **400**，**不得返回 401**。
客户端把 401 解释为「令牌失效」并据此清除登录态；若回 401，
...
> **实现约束（2026-09-11 补，来自实测缺陷）**：「再来一轮」**必须导航到一个全新的
> 练习页**，不得回调原练习页的 State。
>
> 原因：总结页是练习页用 `pushReplacement` 换出来的，练习页此时**已被销毁**；
> 回调到它的 State 上时，`_startRound()` 开头的 `if (!mounted) return;`
> 会让整件事**静默失效** —— 按钮点了没有任何反应，还会白发一次网络请求。
> 本约束曾真实存在过：总结页的「再来一轮」长期无效，而错题页则完全缺失该按钮。
>
> 回归测试见 `app/test/ui/another_round_test.dart`。
