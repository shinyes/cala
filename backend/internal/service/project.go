...
//
// 背景（实测取证，见 evidence/p7 §6.6）：文本答案在服务端是可分类、可判分的，
// 但**客户端没有任何输入途径** —— 练习页只提供自带数字键盘
// （`0-9` `.` `-` 与 `⌫`，规格 §9.3），且刻意不唤起系统键盘（功能6）。
// 因此 `a: "质数"` 这类规则能保存成功，却让用户**永远答不对**。
//
// 第二个拒绝项是**分数形式**（见下），两者同源：
// 都遵循同一条原则 —— **作者写的答案必须能用练习键盘原样敲出来**。
//
// 这是产品层约束，不是判分语义：因此放在 service 层，
// 而 **scoring.Classify 必须继续接受文本与分数** ——
// 已落库的历史信封仍要靠它判分，收窄它会让历史数据无法判定。
//
...
func classifyAnswerable(a string) (scoring.Envelope, error) {
	...
	// 分数形式：只接受整数或小数。
	//
	// 判据是**清洗后的原始字面量是否含 `/`**，而不是看信封 ——
	// `0.5` 与 `1/2` 会生成**数值相等但形状不同**的信封（5/10 与 1/2），
	// 按信封判断既不可靠也无法区分「作者写的是分数」与「作者写的是小数」。
	// 用 Clean 之后的结果判断，可一并覆盖全角 `／`（清洗表把它映射为 `/`）。
	//
	// 为什么值得收窄：练习键盘没有 `/`，因此分数形式里
	// **无限循环小数**（如 `1/3`）永远敲不出来，作者必须自行配容差才行 ——
	// 这是个容易忽略的坑。只允许整数与小数后，
	// 「作者写的答案一定能用键盘原样敲出」这条不变式就**无条件成立**了。
	if strings.ContainsRune(scoring.Clean(a), '/') {
		return scoring.Envelope{}, fmt.Errorf(
			"答案是分数形式 %q，但本题型只接受整数或小数："+
				"练习键盘没有除号，请改写为等值小数（如 1/2 写作 0.5）；"+
				"若该值无法写成有限小数（如 1/3），请改用近似小数（如 0.33）并配置容差", a)
	}
